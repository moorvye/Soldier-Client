...
class SettingsActivity : BaseActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        ...
        private val fragmentPreset by lazy { findPreference<ListPreference>("pref_fragment_preset") }
        ...
        // CDN IP Spoof
        private val enableSniSpoof by lazy { findPreference<CheckBoxPreference>("pref_enable_sni_spoof") }
        private val spoofIp by lazy { findPreference<EditTextPreference>("pref_spoof_ip") }
        private val spoofPort by lazy { findPreference<EditTextPreference>("pref_spoof_port") }

        private val enableCdnSpoof by lazy { findPreference<CheckBoxPreference>("pref_enable_cdn_spoof") }
        private val cdnDomain by lazy { findPreference<EditTextPreference>("pref_cdn_domain") }
        private val enableDnsResolver by lazy { findPreference<CheckBoxPreference>("pref_enable_dns_resolver") }
        private val dnsResolver by lazy { findPreference<EditTextPreference>("pref_dns_resolver") }
        ...
        override fun onCreatePreferences(bundle: Bundle?, s: String?) {
            ...
            fragmentPreset?.setOnPreferenceChangeListener { pref, newValue ->
    val index = (newValue as? String)?.toIntOrNull() ?: 0
    val presets = listOf(
        Triple("1-1", "0-0", "10"),       // 0 خیلی سبک
        Triple("1-3", "0-1", "10"),       // 1 خیلی سبک ۲
        Triple("10-20", "1-5", "10"),     // 2 سبک
        Triple("5-15", "0-2", "10"),      // 3 سبک ۲
        Triple("15-30", "2-6", "10"),     // 4 سبک ۳
        Triple("20-40", "5-10", "10"),    // 5 متوسط
        Triple("30-60", "5-12", "10"),    // 6 متوسط ۲
        Triple("40-80", "8-15", "12"),    // 7 متوسط ۳
        Triple("50-100", "10-20", "10"),  // 8 سنگین
        Triple("60-120", "12-25", "12"),  // 9 سنگین ۲
        Triple("100-200", "15-30", "15"), // 10 خیلی سنگین
        Triple("200-400", "20-50", "20")  // 11 حداکثر
)
    val (length, interval, maxSplit) = presets.getOrElse(index) { presets[0] }

    MmkvManager.encodeSettings(AppConfig.PREF_FRAGMENT_LENGTH, length)
    MmkvManager.encodeSettings(AppConfig.PREF_FRAGMENT_INTERVAL, interval)
    MmkvManager.encodeSettings(AppConfig.PREF_FRAGMENT_MAXSPLIT, maxSplit)

    fragmentLength?.text = length
    fragmentLength?.summary = length
    fragmentInterval?.text = interval
    fragmentInterval?.summary = interval
    fragmentMaxSplit?.text = maxSplit
    fragmentMaxSplit?.summary = maxSplit

    // آپدیت summary خود اسپینر
    val lp = pref as ListPreference
    val idx = lp.findIndexOfValue(newValue as? String)
    lp.summary = if (idx >= 0) lp.entries[idx] else newValue?.toString()
    ...
    true
            ...
            enableSniSpoof?.setOnPreferenceChangeListener { _, newValue ->
                updateSniSpoof(newValue as Boolean)
                true
            }

            enableCdnSpoof?.setOnPreferenceChangeListener { _, newValue ->
                updateCdnSpoof(newValue as Boolean)
                true
            }

            enableDnsResolver?.setOnPreferenceChangeListener { _, newValue ->
                updateDnsResolver(newValue as Boolean)
                true
            }
        }
        ...
        override fun onStart() {
            ...
            updateSniSpoof(MmkvManager.decodeSettingsBool("pref_enable_sni_spoof", false))

            updateCdnSpoof(MmkvManager.decodeSettingsBool("pref_enable_cdn_spoof", false))

            updateDnsResolver(MmkvManager.decodeSettingsBool("pref_enable_dns_resolver", false))
        }
...
//            localDnsPort?.isEnabled = enabled
        ...
        private fun updateFragment(enabled: Boolean) {
            fragmentPreset?.isEnabled = enabled
        ...
        private fun updateSniSpoof(enabled: Boolean) {
            spoofIp?.isEnabled = enabled
            spoofPort?.isEnabled = enabled
        }

        private fun updateCdnSpoof(enabled: Boolean) {
            cdnDomain?.isEnabled = enabled
        }

        private fun updateDnsResolver(enabled: Boolean) {
            dnsResolver?.isEnabled = enabled
        }
