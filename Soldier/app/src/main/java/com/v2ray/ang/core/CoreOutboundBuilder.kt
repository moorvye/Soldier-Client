...
//        val key = profileItem.quicKey
        ...
        // Advanced Fragment Strategy
        val enableFragment = MmkvManager.decodeSettingsBool(AppConfig.PREF_FRAGMENT_ENABLED, false)
        val fragmentStrategy = MmkvManager.decodeSettingsString("pref_fragment_strategy", "sni_split")

        if (enableFragment) {
            LogUtil.i(AppConfig.TAG, "Fragment Strategy: $fragmentStrategy")
            // Here we can apply fragment logic similar to your Go code
        }
...
//                    }
    ...
    var sni = if (profileItem.sni.isNullOrEmpty()) {
        when {
    ...
    // SNI Spoof logic
    val enableSpoof = MmkvManager.decodeSettingsBool("pref_enable_sni_spoof", false)

if (enableSpoof) {
    val spoofSni = MmkvManager.decodeSettingsString("pref_spoof_sni", "").orEmpty()
    val spoofIp = MmkvManager.decodeSettingsString("pref_spoof_ip", "").orEmpty()

    if (spoofSni.isNotEmpty()) {
        sni = spoofSni
        LogUtil.i(AppConfig.TAG, "SNI Spoof applied: SNI=$spoofSni, IP=$spoofIp")
    }
}
